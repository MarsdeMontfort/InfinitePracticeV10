...
const CATEGORY_COUNTS = ALL_CATEGORIES.map((cat) => ({
  name: cat,
  count: ALL_QUESTIONS.filter((q) => q.category.includes(cat)).length,
}));
const QBANK_SIZE = ALL_QUESTIONS.length;

// DIAGNOSTIC LOGIC: Weighted selection by Qbank proportions
function getDiagnosticCategoryAllocation() {
  const alloc = [];
  let totalAllocated = 0;
  CATEGORY_COUNTS.forEach(({ name, count }, i) => {
    const num = Math.floor((count / QBANK_SIZE) * 30);
    alloc.push({ name, num, idx: i });
    totalAllocated += num;
  });
  let remainders = CATEGORY_COUNTS.map(({ name, count }, i) => ({
    name,
    idx: i,
    remainder: ((count / QBANK_SIZE) * 30) % 1,
  }));
  let left = 30 - totalAllocated;
  remainders.sort((a, b) => b.remainder - a.remainder);
  for (let i = 0; i < left; ++i) {
    alloc[remainders[i % remainders.length].idx].num += 1;
  }
  return alloc;
}
function pickDiagnosticQuestions() {
  const alloc = getDiagnosticCategoryAllocation();
  let selected = [];
  alloc.forEach(({ name, num }) => {
    let catPool = ALL_QUESTIONS.filter((q) => q.category.includes(name));
    catPool = [...catPool].sort(() => Math.random() - 0.5);
    selected.push(...catPool.slice(0, num));
  });
  // Remove duplicate Qs (in case of multi-category), trim to 30
  const seen = new Set();
  let result = [];
...
export default function App() {
  ...
  // Diagnostic state (kept separate, does NOT affect Qbank stats)
  const [diagnosticQuestions, setDiagnosticQuestions] = useState([]);
  const [diagnosticAnswers, setDiagnosticAnswers] = useState({});
  const [diagnosticMarked, setDiagnosticMarked] = useState(new Set());
  ...
  const startDiagnostic = () => {
    ...
    setDiagnosticMarked(new Set());
    setDiagnosticIdx(0);
    setStage("diagnostic_quiz");
  };
  ...
  // Diagnostic answer handling (does NOT affect stats)
  const handleDiagnosticAnswer = useCallback(
    (letter) => {
      if (!diagnosticQuestions.length) return;
      const q = diagnosticQuestions[diagnosticIdx];
      if (!q || diagnosticAnswers[q.id]) return;
      const updatedAnswers = { ...diagnosticAnswers, [q.id]: letter };
      setDiagnosticAnswers(updatedAnswers);
    },
    [diagnosticQuestions, diagnosticIdx, diagnosticAnswers]
  );
  ...
  const toggleDiagnosticMark = useCallback(() => {
    if (!diagnosticQuestions.length) return;
    const qid = diagnosticQuestions[diagnosticIdx].id;
    setDiagnosticMarked((prev) => {
      const next = new Set(prev);
      next.has(qid) ? next.delete(qid) : next.add(qid);
      return next;
    });
  }, [diagnosticQuestions, diagnosticIdx]);
  ...
  const nextDiagnostic = () => {
    if (diagnosticIdx + 1 < diagnosticQuestions.length)
      setDiagnosticIdx(diagnosticIdx + 1);
    else setStage("diagnostic_result");
  };
  const prevDiagnostic = () => {
    if (diagnosticIdx > 0) setDiagnosticIdx(diagnosticIdx - 1);
  };
  const exitTest = () => {
  ...
  const diagnosticResults = () => {
    ...
    diagnosticQuestions.forEach((q) => {
      q.category.forEach((cat) => {
        totals[cat] = (totals[cat] || 0) + 1;
        if (diagnosticAnswers[q.id] === q.answer) {
          corrects[cat] = (corrects[cat] || 0) + 1;
        }
      });
    });
  ...
  const getDiagnosticSummaryText = (pct, weakest, strongest) => {
    if (pct >= 85)
      return `Excellent! You have strong overall knowledge. Keep an eye on: ${
        weakest.join(", ") || "—"
      }.`;
    if (pct >= 70)
      return `Solid performance overall, but you could improve in: ${
        weakest.join(", ") || "—"
      }.`;
    if (pct >= 50)
      return `You have some strengths, but should focus on: ${
        weakest.join(", ") || "—"
      }.`;
    return `Significant improvement needed. Start with: ${
      weakest.join(", ") || "—"
    }.`;
  };
  const getDiagnosticRecommendations = (weakest) => {
    if (!weakest.length) return "Great job! No major weak areas detected.";
    return (
      <>
        Focus your review on: <strong>{weakest.join(", ")}</strong>.<br />
        Try creating a custom quiz with these categories.
      </>
    );
  };
  const getColor = (pct) => {
    if (pct >= 70) return "#22c55e"; // green
    if (pct >= 40) return "#fbbf24"; // yellow
    return "#ef4444"; // red
  };
  ...
  useEffect(() => {
    if (stage !== "diagnostic_quiz" && stage !== "diagnostic_review") return;
    const handleKeyDown = (e) => {
      if (!diagnosticQuestions.length) return;
      const q = diagnosticQuestions[diagnosticIdx];
      if (!q) return;
      if (!diagnosticAnswers[q.id] && /^[a-d]$/i.test(e.key)) {
        const choice = e.key.toUpperCase();
        if (q.choices[choice.charCodeAt(0) - 65])
          handleDiagnosticAnswer(choice);
      }
      if (e.key === "ArrowLeft")
        setDiagnosticIdx((idx) => Math.max(0, idx - 1));
      if (e.key === "ArrowRight")
        setDiagnosticIdx((idx) =>
          Math.min(diagnosticQuestions.length - 1, idx + 1)
        );
      if (e.key.toLowerCase() === "m") toggleDiagnosticMark();
    };
    window.addEventListener("keydown", handleKeyDown);
    return () => window.removeEventListener("keydown", handleKeyDown);
  }, [
  ...
  // --- Diagnostic Quiz UI (looks like your normal quiz, but green) ---
  if (stage === "diagnostic_quiz" || stage === "diagnostic_review") {
    const q = diagnosticQuestions[diagnosticIdx];
    const user = diagnosticAnswers[q.id];
    const isMarked = diagnosticMarked.has(q.id);
      ...
      >
        ...
        >
          ...
          <div style={{ display: "flex", gap: "16px", alignItems: "center" }}>
            <span style={{ color: "white", fontSize: "16px" }}>
              Question {diagnosticIdx + 1} of {diagnosticQuestions.length}
            </span>
            <span
              style={{
                color: "white",
            ...
            >
              {
                Object.keys(diagnosticAnswers).filter(
                  (k) => !!diagnosticAnswers[k]
                ).length
              }{" "}
              answered
            </span>
        ...
        <div style={{ maxWidth: "900px", margin: "0 auto", padding: "24px" }}>
          ...
          >
            ...
            >
              <button
                onClick={prevDiagnostic}
                disabled={diagnosticIdx === 0}
                style={{
                  padding: "8px 16px",
                  background: diagnosticIdx === 0 ? "#e5e7eb" : "#16a34a",
                  color: diagnosticIdx === 0 ? "#9ca3af" : "white",
                  border: "none",
                  borderRadius: "6px",
                  cursor: diagnosticIdx === 0 ? "not-allowed" : "pointer",
                  fontWeight: "500",
                }}
              ...
              <button
                ...
                aria-label={
                  diagnosticIdx + 1 < diagnosticQuestions.length
                    ? "Next question"
                    : "Finish"
                }
              >
            ...
            <div style={{ marginBottom: "24px" }}>
              {q.choices.map((choice, idx) => {
                ...
                return (
                  <button
                    key={letter}
                    onClick={() => !user && handleDiagnosticAnswer(letter)}
                    disabled={user != null}
                    style={{
                      display: "block",
                      width: "100%",
  ...
  if (stage === "diagnostic_result") {
    const { totals, corrects } = diagnosticResults();
    const overallCorrect = Object.values(corrects).reduce((a, b) => a + b, 0);
    const percentage = (
      (overallCorrect / diagnosticQuestions.length) *
      100
    ).toFixed(1);

    // Per-category, sorted weakest first
    const catData = Object.keys(totals)
      .map((cat) => {
      ...
      .sort((a, b) => a.catPercent - b.catPercent);

    // Find weakest, strongest, for summary and recs
    const weakest = catData
      .filter((x) => x.catPercent < 70)
      .slice(0, 3)
      .map((x) => x.cat);
    ...
    return (
      ...
      >
        ...
        <div style={{ maxWidth: "900px", margin: "0 auto", padding: "24px" }}>
          ...
          >
            <h2 style={{ marginBottom: "16px", color: "#16a34a" }}>
              Diagnostic Summary
            </h2>
            <div
              style={{
            ...
            <div
              style={{ marginBottom: "18px", fontSize: "16px", color: "#555" }}
            >
              {getDiagnosticSummaryText(+percentage, weakest, strongest)}
            </div>
            <h3 style={{ marginBottom: "8px", color: "#222" }}>
              Category Breakdown
            </h3>
            <div>
              {catData.map(({ cat, catCorrect, catTotal, catPercent }) => (
                <div
                  key={cat}
                  style={{
                    display: "flex",
                  ...
                  >
                    <div
                      style={{
                        width: `${catPercent}%`,
                        height: "100%",
                        background: getColor(catPercent),
                        borderRadius: "4px",
            ...
            <div
              style={{ margin: "18px 0 10px", fontSize: "16px", color: "#222" }}
            >
              <strong>Recommendations:</strong>
            </div>
            <div style={{ marginBottom: "24px", fontSize: "16px" }}>
              {getDiagnosticRecommendations(weakest)}
            </div>
            <div
              style={{
                display: "flex",
            ...
            >
              ...
              <button
                ...
                aria-label="Review Diagnostic Answers"
              >
                REVIEW ANSWERS
              </button>
              <button
